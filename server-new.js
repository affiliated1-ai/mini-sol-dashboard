...
function calcMargin(confidenceScore, regime, recommendedMargin) {
  ...
  // Scale recommended margin by current balance vs $240 baseline
  const scale   = bal / 240;
...
async function handleSignal(sig) {
  ...
  const off       = Math.max(0.05, Math.min(rawOff, 0.20));  // ATR-based pullback
  ...
  try {
    // jupiter.placeLimitOrder returns the PositionRequest PDA string
    // The Position PDA can be derived from owner+pool+custody+side (deterministic)
    // We store both: requestPDA for fill check, positionPDA for monitoring/closing
    ...
    // positionPDA is returned as the requestPDA from our jupiter.js
    // (jupiter.js returns positionRequestPDA.toBase58() from placeLimitOrder)
    // We'll track it and update positionPDA when the request executes
    positionPDA = null;  // populated after fill confirmation
  } catch (err) {
  ...
  const position = {
    requestPDA,          // PositionRequest PDA — used for fill detection
    positionPDA,         // Position PDA — populated after fill, used for close/monitor
    id: requestPDA,      // keep .id for backward compat with state/API
    asset,
    direction,
    side:        isLong ? 'Long' : 'Short',
    margin,
  ...
  // Schedule fill check — keepers typically execute in <5 seconds
  // but we wait up to FILL_TIMEOUT_MS before cancelling
...
async function checkFillOrCancel(requestPDA) {
  ...
  try {
    // In Anchor flow: PositionRequest account is CLOSED by keeper after execution
    // getOrderStatus returns filled=true when account no longer exists
    const st = await jupiter.getOrderStatus(requestPDA);

    if (st.filled) {
      pos.status    = 'open';
      pos.fillPrice = st.fillPrice || pos.entryPrice;  // fallback to limit if oracle price unavailable
      // Note: actual fill price may differ slightly from limit — keeper uses oracle
      // For accurate PnL tracking, query Position account for actual fill price
      ...
      // Timeout — cancel the unfilled request
      try {
        ...
        // If cancel fails, the request may have just been executed — check again
...
async function closePosition(posId, exitPrice, closeReason) {
  // posId can be requestPDA or positionPDA
  const pos = S.positions.find(p => p.requestPDA === posId || p.positionPDA === posId || p.id === posId);
  if (!pos) { log(`closePosition: position ${posId?.slice(0,10)} not found`); return; }

  // Use positionPDA if available, otherwise fall back to requestPDA for the call
  ...
  try {
    ...
    // Position may already be closed by SL/TP — continue to record closure
  }

  // Calculate PnL from known fill and exit prices
...
// ── Position monitor ──────────────────────────────────────────────
// Polls on-chain every MONITOR_INTERVAL ms for SL/TP/liquidation events.
// When a Position account disappears on-chain, the position was closed.
async function monitorPositions() {
  const openPositions = S.positions.filter(p => p.status === 'open');
  if (openPositions.length === 0) return;

  for (const pos of openPositions) {
    // Use positionPDA for monitoring; fall back to requestPDA
  ...
  // Also check pending positions past timeout
...
// Dashboard → reads state (positions, balance, PnL)
